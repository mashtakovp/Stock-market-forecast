...
for train_index, test_index in tscv.split(X):
    ...
    model = Sequential([
        Conv1D(filters=32, kernel_size=2, input_shape=(time_steps, 1)),
        Flatten(),
        Dense(units=1)
    ])
    model.compile(optimizer='adam', loss='mean_squared_error')
    model.fit(X_train, y_train, epochs=50, batch_size=32, verbose=0)  # Сокращенное количество эпох для упрощения
